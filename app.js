...
function portal(){return `<section class="portal"><div class="portal-top"><div class="portal-insignia"><img src="/logo-v14.png" alt="마병대 마크"></div><div class="portal-overline">DANCHU MILITARY COMMAND CENTER</div><h1><em>단츄의</em> 마병대 통합사령부</h1><div class="portal-english">MABYEONGDAE COMMAND PORTAL</div><p class="portal-caption">모든 전력이 하나의 지휘 아래</p></div><div class="sector-grid">
 ...
 ${sector('02','OFFICER DIRECTORY','간부 수첩','부대 편제표와 간부 정보, 아침 점호 및 순검 기록을 확인합니다.','officer','/officers')}
...
function officerNode(o,editable=false){return `<button type="button" class="formation-person ${o.name?'assigned':''}" data-action="${editable?'edit-officer':'view-officer'}" data-id="${esc(o.id)}"><span class="formation-avatar">${o.photoUrl?`<img src="${esc(avatarSrc(o.photoUrl))}" data-profile-img alt="" referrerpolicy="no-referrer">`:svg('officer')}</span><span class="formation-person-text"><small>${esc(o.title)}</small><strong>${esc(o.name||'미등록')}</strong>${officerRankLine(o)}</span>${editable?'<span class="formation-edit">편집 ↗</span>':''}</button>`;}
...
function officers(){return `<section class="section-page">${banner('02 / OFFICER DIRECTORY','간부 수첩','중대 지휘 계통 · 간부 편제 · 아침 점호 및 순검 대사')}
...
function showOfficer(o,readOnly=false){const body=`<div class="info-box mb-20"><strong>${esc(o.group)} / ${esc(o.title)}</strong><br>간부 편제에 고정된 직책입니다.</div>${readOnly?`<h3>${esc(o.name||'미등록')}</h3><p class="officer-rank-detail">${officerRankLine(o)}</p><p>${esc(o.note||'등록된 소개가 없습니다.')}</p>${personLink(o.soopUrl,'SOOP 방송국 방문 ↗')}`:`<form id="officer-form"><input type="hidden" name="id" value="${o.id}">${soopFields(o.soopUrl)}<div class="field-grid">${field('간부 이름 (닉네임)','name',o.name)}${selectField('계급장 선택','insignia',[['','선택 안 함'],...INSIGNIA_OPTIONS.map(x=>[x,x])],o.insignia||o.rank||'')}</div>${rankInsigniaPicker(o.insignia||o.rank||'')}${field('프로필 사진 주소 (자동 입력 또는 직접 입력)','photoUrl',o.photoUrl,false,'url')}${textarea('소개 및 특이사항','note',o.note)}${formButtons('편제 저장')}</form>`}`;modal(`${o.group} · ${o.title}`,body)}
