...
// Owner/admin sessions only: a viewer login is not an administrator.
const isAdminUser = u => u?.role === 'owner' || u?.role === 'admin';
...
function requireAdmin(req,res,owner=false){const u=userFrom(req);if(!isAdminUser(u)||(owner&&u.role!=='owner')){problem(res,403,'접근 권한이 없습니다.');return null;}return u;}
