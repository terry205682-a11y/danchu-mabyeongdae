...
function showSetup(){modal('최초 관리자 설정',`<div class="info-box mb-20">최초 실행 시 서버 터미널에 표시된 <strong>설치 키</strong>로 최고 관리자 계정을 한 번만 생성합니다. 서버가 출력한 키를 확인해 주세요.</div><form id="setup-form">${field('최초 설치 키','setupKey','',true,'text','서버 실행 화면에 표시된 키')}${field('관리자 아이디 (영문 소문자·숫자 3~24자)','username','',true)}${field('비밀번호 (15자 이상)','password','',true,'password')}${formButtons('시스템 활성화')}</form>`)}
...
function showAddUser(){modal('공동 관리자 추가',`<form id="user-form">${field('로그인 아이디','username','',true)}${field('비밀번호 (15자 이상)','password','',true,'password')}${selectField('계정 권한','role',[['admin','공동 관리자 (편집 가능)'],['viewer','열람용 (관리 권한 없음)']],'admin')}${formButtons('계정 생성')}</form>`)}
